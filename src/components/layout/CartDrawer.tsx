            ...
            <div className="space-y-6">
              {items.map(item => (
                <div key={`${item.id}-${item.size}`} className="flex gap-4">
                  {/* Placeholder image */}
                  <div className="w-20 h-24 bg-gray-100 flex-shrink-0 flex items-end justify-center pb-2">
                  ...
                  <div className="text-right">
                    <p className="text-[13px]">${item.price * item.qty}</p>
                    <button
        ...
        {items.length > 0 && (
          <div className="px-6 py-5 border-t border-border space-y-3">
            ...
            <Link
              href="/checkout"
              onClick={closeCart}
              className="block w-full bg-ink text-paper text-[11px] tracking-widest uppercase text-center py-4 hover:bg-gray-900 transition-colors"
            >
              Checkout →
            </Link>
            <Link
              href="/cart"
              onClick={closeCart}
              className="block w-full border border-border text-[11px] tracking-widest uppercase text-center py-3 text-muted hover:border-ink hover:text-ink transition-colors"
            >
              View Cart
            </Link>
          </div>
